...
"""
to be noted: this code was taken from the first assignment of experimental robotics. Necessary changes were made: the markers are detected and followed indiviidually 
this means that the robot now does not need to shut down when all the markers are found. The order of the markers does not matter since we have the waypoints
and we know the position of each marker so we know in advance which marker will be detected. 
...
class MarkerDetector:
    ...
    def __init__(self):
        ...
        #we want to set up an Action Server to be able to send goals and then this will trigger runcontrollofic 
        #Client will use /send_goal to connect and send action requests to start trackin the markers 
        self.actionserver = actionlib.SimpleActionServer('/send_goal', assignment2_exprob.msg.lookformarkerAction, self.runControlLogic, auto_start=False)
    ...
    def runControlLogic(self, aruco_ID):
        try:
            # Get the first marker that hasn't been detected yet
            marker = next((key for key, values in markerDetected_dict.items() if not values), None)
        ...
        #searchign state, the robot is looking for the markers 
        while self.robotState == "Searching":
            ...
            # Check if any of the desired markers is detected if they are either 11, 12, 13 or 15
            detected_marker_ids = [id for id in self.ids_list if id in [11, 12, 13, 15]]

            if detected_marker_ids:
                print("Correct Marker detected:", detected_marker_ids[0])
                self.move_robot(0.0, 0.0, 0.1)
                self.robotState = robotState_dict[2]
            else:
                # Rotate slightly so that the robot can detect the id of the marker 
                self.move_robot(0.0, 0.7, 1.0)
                print("THE ROBOT SHOULD ROTATE SLOWLY")
        

        # moving state, in this case the id of the marker was already detected and now the robot should move slowly towards the marker         
